/* Generated by RuntimeBrowser
   Image: /Library/Developer/CoreSimulator/Volumes/iOS_21A328/Library/Developer/CoreSimulator/Profiles/Runtimes/iOS 17.0.simruntime/Contents/Resources/RuntimeRoot/System/Library/Frameworks/Photos.framework/Photos
 */

@interface PHPlaceholderImageHelper : NSObject

+ (id)placeholderImageDataForImageSize:(struct CGSize { double x1; double x2; })arg1 qualityClass:(id)arg2 dataSpecification:(struct PHAssetResourceTableDataSpecification { int x1; int x2; int x3; int x4; int x5; int x6; long long x7; }*)arg3;

@end
